...
class ScribeSessionManager:
    ...
    def get_token(self) -> str | None:
        ...
        try:
            # Synchronous HTTP request to get the token, since the client may block event loop we should ideally use AsyncClient
            # Changed to httpx.Client() but might be better as httpx.AsyncClient()
    ...
    async def handle_transcript(self, data):
        try:
            ...
            if self.seg_start_time is None:
                self.seg_start_time = datetime.now(timezone.utc)
            ...
            if not partial:
                ...
                if (now - self.last_partial_time).total_seconds() > REALTIME_SETTINGS.get('PARTIAL_INTERVAL', 2):
    ...
    async def start(self):
        ...
        try:
            # To avoid blocking event loop, use asyncio.to_thread for get_token which uses synchronous httpx
